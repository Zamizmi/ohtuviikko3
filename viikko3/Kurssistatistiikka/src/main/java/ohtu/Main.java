...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        int courseHours = 0;
        int courseExercises = 0;
        for (int i = 0; i < statsObj.size(); i++) {
            courseHours += statsObj.get((i + 1) + "").getAsJsonObject().get("hour_total").getAsInt();
            courseExercises += statsObj.get((i + 1) + "").getAsJsonObject().get("exercise_total").getAsInt();
        }
        System.out.println("\nKaikkien kurssilaisten tehtäväpalautuksia: " + courseExercises + " sekä käytetyt tunnit:  " + courseHours);
    }
}
